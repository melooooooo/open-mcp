...
// 服务端函数 - 获取职位站点合集（READ: career_platform.job_sites，带 RLS 公共只读策略）
export async function getJobs() {
  ...
  if (error) {
    console.error('Error fetching job sites:', JSON.stringify(error, null, 2))
    return []
  }
...
// 客户端函数 - 增加浏览量（对应 career_platform.job_sites）
