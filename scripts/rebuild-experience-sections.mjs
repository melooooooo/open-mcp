#!/usr/bin/env node
/**
 * Rebuild finance_experiences.sections using a better split strategy.
 * - Prefers markdown sections when they are meaningful
 * - Falls back to HTML sections when markdown lacks headings
 *
 * Usage:
 *   node scripts/rebuild-experience-sections.mjs --dry-run
 *   node scripts/rebuild-experience-sections.mjs --execute
 *   node scripts/rebuild-experience-sections.mjs --execute --limit=50
 *   node scripts/rebuild-experience-sections.mjs --execute --slug=example-slug
 */
...
const shouldUpdateSections = (existingSections, nextSections) => {
  if (!nextSections || nextSections.length === 0) return false
  if (!existingSections || existingSections.length === 0) return true

  if (existingSections.length <= 1 && nextSections.length > 1) return true

  const existingStats = scoreSections(existingSections)
  const nextStats = scoreSections(nextSections)
  return nextStats.score > existingStats.score
}
...
const main = async () => {
  ...
  for (const row of rows) {
    ...
    const shouldUpdate = force || shouldUpdateSections(existingSections, nextSections)
    if (!shouldUpdate) {
      skipped += 1
      continue
    }
    ...
    if (execute) {
      await client.query(
        "UPDATE finance_experiences SET sections = $1, updated_at = NOW() WHERE id = $2",
        [JSON.stringify(nextSections), row.id]
      )
    }

    updated += 1
    if (!execute) {
      console.log(`[DRY RUN] would update: ${row.slug || row.id} (sections ${existingSections.length} -> ${nextSections.length})`)
    } else {
      console.log(`updated: ${row.slug || row.id} (sections ${existingSections.length} -> ${nextSections.length})`)
    }
  }

  console.log(`done. updated=${updated} skipped=${skipped} total=${rows.length} execute=${execute}`)
  await client.end()
}
